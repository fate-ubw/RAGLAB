...
class FactScorer(object):
    ...
    def get_score(self,
        ...
        if atomic_facts is not None:
            ...
            for topic, gen in tqdm(zip(topics, generations), total = len(generations)):
                ...
                curr_afs, _ = self.af_generator.run(gen) # the key of code     
                curr_afs = [fact for _, facts in curr_afs for fact in facts] # list of string
                if len(curr_afs)==0:
                    atomic_facts.append(None)
                else:
        ...
        init_scores = []
        decisions = []
        for topic, generation, facts in tqdm(zip(topics, generations, atomic_facts), total = len(generations)):
            if facts is None:
                decisions.append(None)
            else:
                decision = self._get_score(topic, generation, facts, knowledge_source) #fact  
                # [{'atom': 'Avraham Eilam-Amzallag is an Israeli politician.', 'is_supported': False}, 
                # {'atom': 'Avraham Eilam-Amzallag serves as a member of the Knesset.', 'is_supported': False}, 
                # {'atom': 'Avraham Eilam-Amzallag serves as a member of the Knesset for the Yisrael Beiteinu party.', 'is_supported': False}]
                score = np.mean([d["is_supported"] for d in decision])
                
                if gamma:
                    init_scores.append(score)
                    penalty = 1.0 if len(facts)>gamma else np.exp(1-gamma/len(facts)) # 
                    # (Pdb) np.exp(1-gamma/10)
                    # 1.0
                    # (Pdb) np.exp(1-gamma/100)
                    # 2.45960311115695
                    # (Pdb) np.exp(1-gamma/1000)
                    # 2.691234472349262
                    score = penalty * score # 
    ...
    def _get_score(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None):
        ...
        for atom in atomic_facts:
            atom = atom.strip()
            if self.lm: # 
                passages = self.retrieval[knowledge_source].get_passages(topic, atom, k=5) #  passages
                definition = "Answer the question about {} based on the given context.\n\n".format(topic)
                context = "" 
                for psg_idx, psg in enumerate(reversed(passages)): #     atom =5。
                    context += "Title: {}\nText: {}\n\n".format(psg["title"], psg["text"].replace("<s>", "").replace("</s>", ""))
                definition += context.strip() # 
                #'Answer the question about Avraham Eilam-Amzallag based on the given context.\n\nTitle: Avraham Eilam-Amzallag\nText: Avraham Eilam-Amzallag Avraham Eilam-Amzallag (; born 28 September 1941) is an Israeli musician and composer.Biography. Avraham (Avi) Eilam-Amzallag was born in Casablanca, Morocco. He immigrated to Israel as a child. He first studied the flute and later compositional studies at the Buchman-Mehta Music Academy in Tel Aviv.Music career. He is the founder of an ensemble that performs oriental music. His compositions are described as a fusion of oriental Jewish music with contemporary technique.Selected compositions. - "Desolation" [3:33 minutes] - (composition for flute) (Mawal) [5:13 minutes] - Music for flute and percussion (1975) - "Composition"Published works. - "Music for flute and percussion" (1975).See also. - Music of Israel'
                if not definition[-1] in string.punctuation:# (Pdb) string.punctuation = '!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~'
                    definition += "."
                prompt = "{}\n\nInput: {} True or False?\nOutput:".format(definition.strip(), atom.strip())
                # 'Answer the question about Avraham Eilam-Amzallag based on the given context.\n\nTitle: Avraham Eilam-Amzallag\nText: Avraham Eilam-Amzallag Avraham Eilam-Amzallag (; born 28 September 1941) is an Israeli musician and composer.Biography. Avraham (Avi) Eilam-Amzallag was born in Casablanca, Morocco. He immigrated to Israel as a child. He first studied the flute and later compositional studies at the Buchman-Mehta Music Academy in Tel Aviv.Music career. He is the founder of an ensemble that performs oriental music. His compositions are described as a fusion of oriental Jewish music with contemporary technique.Selected compositions. - "Desolation" [3:33 minutes] - (composition for flute) (Mawal) [5:13 minutes] - Music for flute and percussion (1975) - "Composition"Published works. - "Music for flute and percussion" (1975).See also. - Music of Israel.
                # \n\nInput: Avraham Eilam-Amzallag is an Israeli politician. True or False?\nOutput:'
                ...
                output = self.lm.generate(prompt) #  cache
                ...
                if type(output[1])==np.ndarray:
                    ...
                    if "true" in generated_answer or "false" in generated_answer:
                        ...
                        is_supported = all([keyword not in generated_answer.lower().translate(str.maketrans("", "", string.punctuation)).split() for keyword in ["not", "cannot", "unknown", "information"]]) #如果没有生成的话，那么就打一个补丁，然后
...
if __name__ == '__main__':
    ...
    # dataset_name = os.path.splitext(input_file_name)[0]
    # output_file_name = f"{dataset_name}-factscore_output-{model_name}.jsonl" 
    #
    # output_folder = os.path.dirname(args.input_path)    
    # # chrck output file exist
    # output_file = os.path.join(output_folder, output_file_name)

    # out = np.array(out, dtype=bool).tolist()
